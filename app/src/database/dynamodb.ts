...
export class DynamoDBService {
  ...
  constructor() {
    ...
    this.notificationTable = process.env.NOTIFICATION_TABLE || 'notification-table';
    this.notificationErrorTable = process.env.NOTIFICATION_ERROR_TABLE || 'notification-error-table';
  }
  ...
  async saveNotificationError(errorRecord: any): Promise<void> {
    try {
      ...
      throw new Error(`DynamoDB error save failed: ${(error as Error).message}`);
    }
  }
  ...
  async getNotificationHistory(userId: string, limit: number = 50): Promise<NotificationRecord[]> {
    try {
      ...
      return response.Items?.map(item => unmarshall(item) as NotificationRecord) || [];
    } catch (error) {
